...
namespace MyBankAccount.Bank
{
    public class Registration:Customer
    {
        ...
        public void RegisterNewAccount(string connectionString, string firstName, string lastName, string phone, string email, string userName, string pass, string typeOfCust)
        {
            ...
            string sql = "insert into Customers (FirstName, LastName, Phone, Email, TypeOfCust, Login, Password, RegisterDate) values " + $"('{FirstName}', '{LastName}', '{Phone}', '{EMail}', '{TypeOfCustomer}', '{UserName}', '{Password}', '{RegisterDate}' )";
            try
            {
                _sqlConnection.Open();
